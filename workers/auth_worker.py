    ...
    def run(self):
        while self.running:
            ...
            try:
                ...
                print(f"[AuthWorker] DeepFace exception: {e}")
                verified = False
            ...
            # Yield control - Auth runs at 0.1 FPS, so sleep 10s or check thread state periodically
            # Sleeping in smaller increments allows the thread to stop quickly if requested
            for _ in range(100):
                if not self.running:
                    break
                time.sleep(0.1)
